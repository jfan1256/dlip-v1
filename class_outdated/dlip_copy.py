...
from models.dlip import create_vit, init_tokenizer, load_checkpoint

class DLIP_Pretrain(nn.Module):
    def __init__(self,                 
        ...
        text_width = self.text_encoder.bert_config.hidden_size
    ...
    def forward(self, image, caption, teacher, alpha):
        ...
        with torch.no_grad():
            ...
            image_embeds_tea_avg = image_embeds_tea[:,1:,:].mean(dim=1, keepdim=True)
            image_embeds_tea_avg = image_embeds_tea_avg.view(bs, -1)
            image_feat_tea_avg = F.normalize(teacher.vision_proj(image_embeds_tea_avg[:,1:,:]),dim=-1) 
        ...
        loss_kl = (1.0/bs_logits)*self.criterion_kl(F.log_softmax(logits/temp, dim=1),F.softmax(logits_tea/temp, dim=1))
        ...
        loss_Sattn = (1.0/bs_atten)*self.criterion_kl(F.log_softmax(attention_score/temp_score, dim=1),F.softmax(attention_score_tea/temp_score, dim=1))
        ...
        loss_Xattn = (1.0/bs_atten)*self.criterion_kl(F.log_softmax(x_attention_score/temp_score, dim=1),F.softmax(x_attention_score_tea/temp_score, dim=1))
        ...
        loss_DXattn = (1.0/bs_atten)*self.criterion_kl(F.log_softmax(dx_attention_score/temp_score, dim=1),F.softmax(dx_attention_score_tea/temp_score, dim=1))
